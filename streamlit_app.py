...
st.set_page_config(page_title="TIMEFLIP Engine", layout="wide")
...
.badge-strongsell{background:#7a1a1a;border-radius:6px;padding:2px 12px;font-size:0.75rem;
                  font-weight:700;color:white}
.badge-lora{background:#8e44ad;border-radius:6px;padding:2px 8px;font-size:0.65rem;
            font-weight:700;color:white}
</style>
""", unsafe_allow_html=True)

st.markdown('<div class="main-header">🔄 TIMEFLIP Engine</div>',
            unsafe_allow_html=True)
st.markdown(
    '<div class="sub-header">Foundation Model Finetuner · LoRA Adapter · '
    'Parameter-Efficient Time-Series Foundation Models</div>',
    unsafe_allow_html=True)
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
st.sidebar.markdown("## 🔄 TIMEFLIP")
st.sidebar.markdown(f"**Next Trading Day**")
st.sidebar.markdown(f"`{next_trading_day()}`")
st.sidebar.markdown(f"**Model:** {config.MODEL['model_type']}")
st.sidebar.markdown(f"**LoRA Rank:** {config.LORA['rank']}")
st.sidebar.markdown(f"**Context Len:** {config.MODEL['context_len']}")
st.sidebar.markdown(f"**Horizon:** {config.MODEL['horizon_len']}")
st.sidebar.markdown("---")
...
tab1_path = find_latest(files, "timeflip_")
tab2_path = find_latest(files, "timeflip_breakdown_")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.header("🏆 Best Window per ETF — TIMEFLIP Forecast Signal")

    with st.expander("📖 How TIMEFLIP Works", expanded=True):
        st.markdown("""
**Time-Series Foundation Model Finetuner (TIMEFLIP)** adapts pre-trained models to financial data:

| Step | What happens |
|------|-------------|
| 1. Foundation Model | Load pre-trained TimesFM/Moirai/Chronos |
| 2. LoRA Adapter | Inject lightweight low-rank adapters |
| 3. Finetuning | Adapt to your ETF universe with minimal parameters |
| 4. Forecasting | Generate domain-specific forecasts |

**Interpretation:**
- **Higher z-score** → Stronger foundation model forecast
- **LoRA Parameters** → Number of trainable parameters (< 2% of full model)
...
<div class="hero-card">
  ...
  <div class="score">Finetuned: {'✅' if finetuned else '❌'}</div>
...
""", unsafe_allow_html=True)
            ...
            if full_scores:
                ...
                for t, info in full_scores.items():
                    ...
                    rows.append({
                        ...
                        "Finetuned": "✅" if info.get("finetuned", False) else "❌",
                        "LoRA Params": int(safe_float(info.get("trainable_params", 0))),
                ...
                styled_df = df_rank.style.map(
                    lambda x: 'background-color: #27ae60; color: white;' if isinstance(x, (int, float)) and x > 0.1 else '',
                    subset=['z-score']
                ).map(
                    lambda x: 'background-color: #f1c40f; color: black;' if isinstance(x, (int, float)) and -0.1 < x <= 0.1 else '',
                    subset=['z-score']
                ).map(
                    lambda x: 'background-color: #e74c3c; color: white;' if isinstance(x, (int, float)) and x <= -0.1 else '',
                    subset=['z-score']
                )
    ...
    st.caption(f"Run date: {data1.get('run_date','?')} · Higher z-score = stronger foundation model forecast")
...
# ══════════════════════════════════════════════════════════════════════════════
with tab2:
    st.header("🔍 Explore TIMEFLIP by Window")
    ...
    with st.expander("ℹ️ Window guidance", expanded=False):
        st.markdown("""
- **63d** — Short-term foundation model adaptation
- **126d** — Medium-term adaptation
- **252d** — Annual adaptation: recommended primary
- **504d** — Long-term adaptation
        """)

    st.markdown(f"### TIMEFLIP Rankings at **{win_labels.get(selected_win, f'{selected_win}d')}** window")
