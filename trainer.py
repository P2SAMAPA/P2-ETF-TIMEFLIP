"""
trainer.py  —  Orchestrator for TIMEFLIP Engine
================================================

Loads data → finetunes foundation model → forecasts → ranks ETFs → builds JSON.
...
from timeflip import compute_universe_timeflip
...
def process_window(args: Tuple) -> Dict:
    ...
    try:
        ...
        result = compute_universe_timeflip(universe_prices, macro_df, config_dict, window)
...
def run_trainer(hf_token: Optional[str] = None) -> Dict:
        ...
        for ticker in available:
            ...
            if best_win is not None:
                best_window_per_etf[ticker] = {
                    ...
                    "finetuned": best_data.get("finetuned", False),
                    "trainable_params": int(safe_float(best_data.get("trainable_params", 0))),
    ...
    logger.info("\n💾 Saving JSON results...")
    tab1_path = f"timeflip_{run_date}.json"
    tab2_path = f"timeflip_breakdown_{run_date}.json"
